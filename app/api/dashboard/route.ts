...
export async function GET() {
  ...
  // Today's predicted sales: average of last 7 days * weather factor (already noisy)
  ...
  const dayOverDayPct = yesterdayQty > 0
    ? Math.round(((todayQty - yesterdayQty) / yesterdayQty) * 1000) / 10
    : 0

  const lowStock = inventory.filter((i) => getStockStatus(i) === "low").length
  const expiringCount = expiring.length

  // 14 day forecast: last 7 days actual + 7 days predicted
  const forecast: Array<{ date: string; actual: number | null; predicted: number }> = []
  // last 7 days (including today) with both actual and predicted
  for (let i = 6; i >= 0; i--) {
  ...
  // next 7 days predicted-only (use weekday average pattern)
  ...
  const lidLow = inventory.find((i) => i.product.includes("플라스틱 뚜껑") && getStockStatus(i) === "low")
  if (lidLow) {
    alerts.push({
      id: alertId++,
      type: "warning",
